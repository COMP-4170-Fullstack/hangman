...
if (!supabaseUrl || !supabaseAnonKey) {
    console.error(
        "ERROR: Configure NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in the .env file",
    );
    console.error(
...
async function ensureProfile(supabase, user) {
    ...
    if (existing) return;
    const username =
        user.user_metadata?.username || "player_" + user.id.slice(0, 8);
...
app.get("/auth/callback", async (req, res) => {
    ...
    if (code) {
        ...
        if (!error && data.session) {
            ...
            const supabaseWithAuth = getSupabaseClient(
...
app.post("/api/update-score", requireAuth, async (req, res) => {
    const { won, wordId, gameScore } = req.body;
    const supabase = getSupabaseClient(req.accessToken);
    ...
    const currentTotal = current?.total_score || 0;
    const thisGameScore = Number(gameScore) ?? (won ? 100 : 0);
    const newTotal = currentTotal + thisGameScore;
